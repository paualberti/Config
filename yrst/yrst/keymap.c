...
enum layer_number { _HOME, _SYM, _NUMH, _BASE, _MAJ, _SIM, _NUMB, _GLOB };

// see https://github.com/ChuseCubr/mini-ryoku for layout details
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [_HOME] = LAYOUT_mask(_HOME_LAYOUT), [_SYM] = LAYOUT_mask(_SYM_LAYOUT), [_NUMH] = LAYOUT_mask(_NUMH_LAYOUT), [_BASE] = LAYOUT_mask(_BASE_LAYOUT), [_MAJ] = LAYOUT_mask(_MAJ_LAYOUT), [_SIM] = LAYOUT_mask(_SIM_LAYOUT), [_NUMB] = LAYOUT_mask(_NUMB_LAYOUT), [_GLOB] = LAYOUT_mask(_GLOB_LAYOUT),
};
